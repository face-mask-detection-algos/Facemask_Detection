...
if __name__ == "__main__":
    ...
    predictions = pd.read_csv(args.predictions).rename(columns={"image_name": "filename"})
    predictions["class"] = predictions["class"].apply(lambda x: 1 if x == "mask" else 0)
    # adjust predictions
    # predictions["x"] = predictions["x"] - predictions["w"]/2
    # predictions["y"] = predictions["y"] - predictions["h"]/2
    ...
    # save results
    for df, variable in zip([df_localization, df_positives, df_negatives], ["localization", "positives", "negatives"]):
